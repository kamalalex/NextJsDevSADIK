...
export default function ClientList() {
    ...
    const fetchRequests = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Filter requests where the source is a CLIENT_COMPANY
                // Note: The requester is the one who initiates.
                // In our model, Client (C) adds Transporter (T).
                // Subcontractor record: transportCompany = C, linkedCompany = T.
                // Request for T: fetch where linkedCompanyId = T. (Already done by API)
                // We just need to check if the transportCompany (requester) is a Client.
                const clientRequests = data.filter((req: any) => req.transportCompany.type === 'CLIENT_COMPANY' || req.transportCompany.type as string === 'CLIENT_COMPANY');
    ...
    const handleRequestAction = async (id: string, action: 'CONFIRM' | 'REJECT') => {
        try {
            const response = await fetch('/api/company/partners/requests', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                fetchRequests();
                fetchClients();
                alert(action === 'CONFIRM' ? 'Client accepté avec succès !' : 'Demande refusée.');
            } else {
                const data = await response.json();
    ...
    const handleLinkSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setLinkCode('');
                fetchClients();
                alert(await (await response.clone()).json().then((data: any) => data.message) || 'Demande envoyée avec succès !');
            } else {
                const data = await response.json();
            ...
            {requests.length > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 space-y-4">
                    <h3 className="text-lg font-medium text-amber-900 flex items-center gap-2">
                        <span>🔔</span> Demandes de clients en attente
                    </h3>
